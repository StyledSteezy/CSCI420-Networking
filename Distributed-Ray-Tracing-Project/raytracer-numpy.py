...
# Quadrant specifies which part of the scene to work on. 0 indicates do the entire scene.
# 1 - 4 starts in the top left and goes clockwise
def render(view, scene, piece, num_pieces):
    if piece == 0:
        ...
        bitmap = np.zeros((view.width, view.height, 3), np.float64)
        for y in range(view.height):
            for x in range((piece-1) * view.width // num_pieces, piece * view.width // num_pieces):
                c = traceRay(Ray(scene.camera.pos, getPoint(view,scene.camera,x,y)), scene, 0)
                bitmap[x,y,:] = c
...
if len(sys.argv) == 5:
    ...
    if r != None:
        ...
        s = time.time() 
        bmp = render(view, scene, int(sys.argv[3]), int(sys.argv[4]))
        e = time.time()
    ...
    if r != None:
        ...
        print(f"Lights: {len(scene.lights)}")
        s = time.time() 
        bmp = render(view, scene, 0, 0)
        e = time.time()
        print(f"Done {e - s} seconds")
        writePPM(sys.argv[2],bmp)
elif len(sys.argv) == 2 and sys.argv[1] == "--demo":
    bmp = render(Viewport(400,400), demoScene, 0, 0)
    writePPM("demo.ppm", bmp)
else:
    print(f"Usage: {sys.argv[0]} [scene file] [output ppm] [piece to generate] [num pieces]")
